<template>
  <div
    class="wrapper flex flex-col justify-center items-center w-full min-h-screen max-h-screen bg-[#F2FFF5] leading-[13rem]"
  >
    <div
      class="flex items-center gap-3 p-2 rounded-full border-2 border-[#153047] text-xs font-bold"
      :class="{
        'bg-[#51da6cc8]': long.break == 2,
        'bg-[#4cabff4a]': long.break == 1,
      }"
    >
      <svg
        width="29"
      ...
      <p v-if="long.break == 1">Long Break</p>
      <p v-else>Short Break</p>
    </div>
    <div
      :class="{
        'text-[#14401D]': long.break == 2,
        'text-[#153047]': long.break == 1,
      }"
    >
      <h1 class="text-[15rem] font-bold">
        <span class="text-[15rem] font-bold" v-if="long.min < 10">0</span
        >{{ long.min }}
      </h1>
      <h1 class="text-[15rem] font-bold">
        <span class="text-[15rem] font-bold" v-if="long.s < 10">0</span
        >{{ long.s }}
      </h1>
    </div>
    <div class="flex items-center p-5 gap-3 text-2xl">
      <i
        class="bx bx-dots-horizontal-rounded p-4 rounded-2xl cursor-pointer"
        :class="{
          'bg-[#51da6cc8]': long.break == 2,
          'bg-[#4cabff4a]': long.break == 1,
        }"
      ...
      <i
        v-if="long.pause"
        @click="play('play')"
        class="bx bx-pause px-8 py-5 rounded-3xl cursor-pointer"
        :class="{
          'bg-[#42e162f9]': long.break == 2,
          'bg-[#4cabffaf]': long.break == 1,
        }"
      ></i>
      <i
        v-else
        @click="play('pause')"
        class="bx bx-play px-8 py-5 rounded-3xl cursor-pointer"
        :class="{
          'bg-[#42e162f9]': long.break == 2,
          'bg-[#4cabffaf]': long.break == 1,
        }"
      ...
      <i
        @click="next('next')"
        class="bx bx-skip-next p-4 rounded-2xl cursor-pointer"
        :class="{
          'bg-[#51da6cc8]': long.break == 2,
          'bg-[#4cabff4a]': long.break == 1,
        }"
...
function play(val) {
  ...
  if (val == "play" && long.timer == "pause") {
    long.timer = "play";
    start = setInterval(() => {
      if (long.s > 0) {
        long.s -= 1;
      }
      if (long.s <= 0 && long.min > 0) {
        long.min -= 1;
        long.s = 59;
      }
      if (long.s == 0 && long.min == 0) {
...
function next(val) {
  ...
  if (long.break == 1) {
    ...
    long.s = 0;
    long.break = 2;
    const audio = new Audio("../public/shortbreak.mp3");
    audio.play();
  } else {
    long.min = 25;
    long.s = 0;
